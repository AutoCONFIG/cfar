function [TDR, FAR, true_detections, false_alarms, false_alarm_positions] = perf_cfar(xc, XT, index, target_positions, adjacent_window_size)
    ...
    % adjacent_window_size: 控制相邻位置的范围（可以调节）
            ...
            if ~detected_positions(idx)
                ...
                if all(~detected_positions(start_idx:end_idx))
                    ...
                    if ismember(idx, target_positions)
                        ...
                        disp(['True Detection at index ', num2str(idx), ', value: ', num2str(abs(xc(idx))), ', Threshold: ', num2str(XT(i))]);
                    else
                        false_alarms = false_alarms + 1;  % 虚警
                        false_alarm_positions = [false_alarm_positions, idx];  % 记录虚警位置
                        disp(['False Alarm at index ', num2str(idx), ', value: ', num2str(abs(xc(idx))), ', Threshold: ', num2str(XT(i))]);
                    end
                    % 标记当前检测位置为已检测
    ...
    % 输出最终的TDR和FAR
    disp(['True Detection Rate (TDR): ', num2str(TDR)]);
    disp(['False Alarm Rate (FAR): ', num2str(FAR)]);
end
